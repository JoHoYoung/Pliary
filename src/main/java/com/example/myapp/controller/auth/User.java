...
public class User {
    ...
    public JSONObject Signup(@RequestBody Signup param)
    {
        ...
        try {
            ...
            // 비밀번호를 암호화하여 DB 에 저장, 회원가입 처리.
            userMapper.userSignup(email, Encoder.encrypt(password), token);
    ...
    public JSONObject signIn(@RequestBody Signin param) {
        ...
        System.out.println(Encoder.encrypt(password));
        System.out.println(userInfo.getPassword());
        if (!Encoder.encrypt(password).equals(userInfo.getPassword())){
            JSON.put("statusCode", 700);
            JSON.put("statusMsg", "Invalid Password");
            return JSON;
        }
